...
export const verifyEmailHandler = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!email || !isValidSyntax(email)) {
      ...
      resultData.subresult = resultData.didyoumean ? 'typo_detected' : 'invalid_syntax';
    ...
    if (mxRecords.length === 0) {
      resultData.subresult = 'no_mx_records';
      res.json(finalize(resultData, startTime));
      return;
    }
    ...
    if (smtpCheck.exists) {
      ...
      resultData.result = smtpCheck.subresult === 'greylisted' ? 'unknown' : 'invalid';
